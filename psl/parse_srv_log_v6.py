...
def plot_analyse():
	...
	for bar in bars:
		...
		plt.text(bar.get_x()+width/2, height+2, int(height),ha='center',va='bottom',size='xx-small',rotation=90)
	# 显示
...
@unique
class LOG_TYPE(Enum): # must be in sequence!
	START = 0
	GPS_TYPE = 1
	TIME = 2
	STOP = 3
...
def check_info_filled(locate_info, info_type_to_fill):
	...
	while True:
		if check_type is LOG_TYPE.RES:
			pass
		elif locate_info[check_type.name] is not None:
			return True
		if check_type.value < len(LOG_TYPE) - 1:
			check_type = LOG_TYPE(check_type.value + 1)
		else:
			break
	return False
...
def plot_locate(locate_info_list):
	...
	if len(loc_unknown_fail) is not 0:
		bars_5 = plt.bar([x[i] for i in loc_unknown_fail], [y[i] for i in loc_unknown_fail], width=width, color='#606060')
	...
	for bar in bars:
		height = bar.get_height()
		plt.text(bar.get_x()+width/2, height+2, '?' if height == 0 else int(height), ha='center', va='bottom', size='xx-small', rotation=90)
	# 显示
	plt.show(block=False)


# 基础解析
def BASIC(inFile = None):
	global file, log, fw, stop, stop_1, stop_all, stop_hold, drv_start, period, start, locate, stop_2, stop_pos, stop_ack, time_fin, agps_start, agps_ok, agps_fail, time_fin_int, locate_info_list, imei, first_date, first_time, last_date, last_time
	...
	for line_no in range(0,len(log)):
		...
		log_type,log_data = parse_line(line_no)
		if log_type is None:
			continue
